<div>
    ...
    <main class="w-full h-full">
        ...
        <div class="mt-4 justify-items-center grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
            @foreach ($notes as $note)
                ...
                <div class="w-[23rem] h-[25rem] p-4 border border-gray-800 bg-zinc-300 rounded-lg shadow flex items-center justify-between flex-col">
                    <h5 class="mt-3 text-2xl font-bold tracking-tight text-gray-900">
                        {{ $note->title }}
                    </h5>
                    ...
                    <p class="mb-3 font-medium text-center text-gray-800"> 
                        {{ Carbon\Carbon::parse($note->date)->format('d/m/Y') }} 
                    </p>
                    
                    <div class="w-full flex items-center justify-end">
                        <button href='#' class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-500 rounded-lg hover:bg-blue-900">
                            Editar <i class="fa-solid fa-pen-to-square text-lg"></i>
                        </button>
